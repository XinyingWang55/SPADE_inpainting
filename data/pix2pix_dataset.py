...
class Pix2pixDataset(BaseDataset):
    ...
    def initialize(self, opt):
        ...
        # for mask generation 
        self.mask_generation = MaskGeneration()
    ...
    def __getitem__(self, index):
        ...
        # instance range [0,255]
        if self.opt.no_instance:
            instance_tensor = 0
        else:
            instance_path = self.instance_paths[index]
            instance = Image.open(instance_path)
            if instance.mode == 'L':  # 灰度，像素8位
        ...
        # mask range [0,1]
        if self.opt.no_mask:
            # in training phase, generate masks            
            mode=RandomAttribute()
            array=np.array(image)
            gt, mask = self.mask_generation(array, mode, verbose=False)  #[0~255], 255 is missing
            mask_tensor = transform_label(Image.fromarray(mask))  #[0~1], 1 is missing     
        else:
            # in val or test phase, read existed masks
